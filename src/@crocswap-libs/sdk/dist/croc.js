...
const tempest_1 = require("./vaults/tempest");
...
class CrocEnv {
    ...
    tempestVault(vaultAddr, token1Addr, strategy) {
        let vaultView = this.tokens.materialize(vaultAddr);
        let token1View = this.tokens.materialize(token1Addr);
        return new tempest_1.TempestVault(vaultView, token1View, strategy, this.context);
    }
    slotReader() {
